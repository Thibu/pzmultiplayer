  ...
  return (
    <Accordion type="single" collapsible className='w-full space-y-3'>
      {steamNews.appnews.newsitems.map((news: NewsItem) => (
        <AccordionItem key={news.gid} value={String(news.gid)} className='bg-gradient-to-b from-primary/95 to-primary/90 text-primary-foreground rounded-lg shadow-lg border-b-0'>
          <AccordionTrigger className="text-primary-foreground">
            <div className="flex flex-col gap-1 px-4">
              <span className="font-semibold">{news.title}</span>
              <span className="text-primary-foreground/80 text-xs font-mono">{formatNewsDate(news.date)}</span>
            </div>
          </AccordionTrigger>
          <AccordionContent>
